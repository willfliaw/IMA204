# From https://github.com/t-suzuki/gradient_vector_flow_test/blob/master/gvf.py
#!env python
# -*- coding: utf-8 -*-
...
if __name__ == "__main__":
    ...
    ax.set_axis_off()
    ax = axs[0][2]
    ax.imshow(fx, vmin=vmin(fx), vmax=vmax(fx), cmap="seismic")
    ax.set_title("fx")
    ax.set_axis_off()
    ax = axs[0][3]
    ax.imshow(fy, vmin=vmin(fx), vmax=vmax(fx), cmap="seismic")
    ax.set_title("fy")
    ax.set_axis_off()
    ax = axs[1][0]
    ax.imshow(gx, vmin=vmin(gx), vmax=vmax(gx), cmap="seismic")
    ax.set_title("GVFx")
    ax.set_axis_off()
    ax = axs[1][1]
    ax.imshow(gy, vmin=vmin(gy), vmax=vmax(gy), cmap="seismic")
    ax.set_title("GVFy")
    ax.set_axis_off()
    ax = axs[1][2]
    plot_vector_field(ax, fx, fy)
    ax.set_title("f")
    ax.set_axis_off()
    ax = axs[1][3]
    plot_vector_field(ax, gx, gy)
    ax.set_title("GVF")
    ax.set_axis_off()
    fig.tight_layout()
